...
public class Homework01 {
    public static void main(String[] args) throws IOException {
        /**
         *(1) 在判断D:\\upload下是否有文件夹mytemp ,如果没有就创建mytemp
         *(2) 在D:\\upload\\mytemp 目录下, 创建文件 hello.txt
         *(3) 如果hello.txt 已经存在，提示该文件已经存在，就不要再重复创建了
         *(4) 并且在hello.txt 文件中，写入 hello,world~

         */

        String directoryPath = "D:\\upload\\mytemp";
        File file = new File(directoryPath);
        if(!file.exists()) {
            //创建
            if(file.mkdirs()) {
                System.out.println("创建 " + directoryPath + " 创建目录成功" );
            }else {
                System.out.println("创建 " + directoryPath + " 创建目录失败");
            }
        }
